...
$sth->finish(); #}
if ($incount) {
$printlist = box_header("$incount New Message(s)");
$printlist .= $pmlist;
$printlist .= box_footer();
return $printlist;
 } else { return ''; }
} else { return ''; }
}
...
# Default
my $menu = "<img src=\"$cfg{themesurl}/$cfg{default_theme}/images/dot.gif\" alt=\"$title\" />&nbsp;<a href=\"$page\" class=\"nav\"$logout>$title</a>";
if ($image && $image1) { # Link & Dot image
$menu = "<img src=\"$cfg{imagesurl}/$image1\" alt=\"$title\" />&nbsp;<a href=\"$page\"$logout><img src=\"$cfg{imagesurl}/$image\" alt=\"$title\" /></a>";
} elsif (!$image1 && $image) { # Link image
$menu = "<img src=\"$cfg{themesurl}/$cfg{default_theme}/images/dot.gif\" alt=\"$title\" />&nbsp;<a href=\"$page\"$logout><img src=\"$cfg{imagesurl}/$image\" alt=\"$title\" /></a>";
} elsif (!$image && $image1) { # Dot image
$menu = "<img src=\"$cfg{imagesurl}/$image1\" alt=\"$title\" />&nbsp;<a href=\"$page\" class=\"nav\"$logout>$title</a>";
}
...
$title = $title
 ...
 : '';
 return $title.'<div class="menuback">'."\n";
}
